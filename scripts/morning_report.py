...
sys.path.insert(0, 'D:/Garmin/Garmin/garmin-agent/GarminAgent')
from login.garmin_login import garmin_login

client = garmin_login(email="240225622@qq.com", password="Ji1ji1ji4", is_cn=True)
today = date.today()
...
for m in rhr_data.get("allMetrics", {}).get("metricsMap", {}).get("WELLNESS_RESTING_HEART_RATE", []):
    if m.get("value"): rhr = int(m["value"])

sd = client.get_sleep_data(y)
if isinstance(sd, list): sd = sd[0] if sd else {}
...
for i in range(7, 0, -1):
    ...
    for m in r.get("allMetrics", {}).get("metricsMap", {}).get("WELLNESS_RESTING_HEART_RATE", []):
        if m.get("value"): rhr_list.append((d, int(m["value"])))
...
for i in range(7, 0, -1):
    ...
    if total: sleep_list.append((d, total, sds.get("deepSleepSeconds", 0) or 0, sds.get("awakeCount", 0) or 0))
...
def score_hrv(last_night, weekly):
    if last_night <= 0 or weekly <= 0: return 75
    cl = math.log(last_night); bl = math.log(weekly)
    p = (cl - bl) / bl
    if p > 0.10: s = 70
    elif p > 0.05: s = 85 + (p - 0.05) * (-300)
    elif p >= 0: s = 75 + p * 200
    elif p >= -0.05: s = 75 + p * 300
    elif p > -0.08: s = 60 + (p + 0.05) * (2000/3)
    elif p > -0.15: s = 40 + (p + 0.08) * (3000/7)
    elif p >= -0.20: s = 10 + (p + 0.15) * 200
    else: s = 0
...
def score_rhr(c, b):
    d = c - b
    if d < -3: s = 95
    elif d <= 0: s = 95 + (d + 3) * (-20/3)
    elif d <= 3: s = 75 + d * (-20/3)
    elif d <= 6: s = 55 + (d - 3) * (-25/3)
    elif d <= 10: s = 30 + (d - 6) * (-7.5)
    else: s = 0
    return round(min(100, max(0, s)))
...
def score_sleep(total_sec, deep_sec, rem_sec, awake_cnt, gs=None):
    ...
    if th < 5: ds = 10
    elif th < 6: ds = 10 + (th - 5) * 35
    elif th < 7: ds = 45 + (th - 6) * 35
    elif th <= 9: ds = 80 + (th - 7) * 10
    else: ds = max(50, 100 - (th - 9) * 20)
    if dp < 5: des = 20
    elif dp < 10: des = 20 + (dp - 5) * 8
    elif dp <= 25: des = 60 + (dp - 10) * (40/15)
    else: des = 90
    if rp < 10: res = 30 + rp * 2
    elif rp < 18: res = 50 + (rp - 10) * 2.5
    elif rp <= 25: res = 70 + (rp - 18) * (30/7)
    else: res = 85
    if awake_cnt == 0: aws = 100
    elif awake_cnt == 1: aws = 80
    elif awake_cnt == 2: aws = 60
    else: aws = max(0, 45 - (awake_cnt - 3) * 15)
...
# ===================== ROOT CAUSE =====================
issues = []
if hrv_pct < -5: issues.append(f"HRV 低于基线 {abs(hrv_pct):.0f}%，自主神经恢复不足")
elif hrv_pct > 10: issues.append(f"HRV 异常升高（>+10%），需关注是否副交感反弹")
if rhr_dev > 3: issues.append(f"RHR 高于基线 {rhr_dev}bpm，交感活性偏高")
if rhr_dev < -3: issues.append(f"RHR 显著低于基线，副交感占优 — 若伴随疲劳感需排查")
if ts < 6*3600: issues.append(f"睡眠时长不足（{ts//60}分钟）")
if awake >= 3: issues.append(f"夜间清醒 {awake} 次，睡眠碎片化")
if dp < 10 and ts > 0: issues.append(f"深睡占比仅 {dp:.0f}%，低于理想范围 10-25%")
if dp < 5 and ts > 0: issues.append(f"深睡严重不足（{dp:.0f}%），可能是压力或浅睡过多")
if rscore < 60: issues.append(f"训练准备度偏低（{rscore}/100）")
if recovery < 60 and recovery >= 40:
    if len([x for x in issues if x]) >= 2: issues.append("多项指标共同指向恢复不足")
elif recovery < 40:
    issues.append("Plews(2013) 框架下 NFOR 风险 — 建议调整训练周期")

positives = []
if hrv_pct >= -5 and hrv_pct <= 5: positives.append("HRV 在正常波动范围内，自主神经平衡")
if hrv_pct > 5 and hrv_pct <= 10: positives.append("HRV 高于基线，恢复状态良好")
if rhr_dev >= -3 and rhr_dev <= 3: positives.append("RHR 稳定，无异常交感激活")
if rhr_dev >= -3 and rhr_dev < 0: positives.append("RHR 略低于基线，副交感占优有益恢复")
if ts >= 7*3600: positives.append(f"睡眠时长充足（{ts//60}分钟）")
if dp >= 10 and dp <= 25: positives.append(f"深睡占比 {dp:.0f}% 在理想范围 10-25%")
if dp >= 15: positives.append(f"深睡占比 {dp:.0f}%，恢复性睡眠质量高")
if rscore >= 80: positives.append(f"训练准备度高（{rscore}/100），身体状态在线")
if rscore >= 60 and rscore < 80: positives.append(f"训练准备度中等（{rscore}/100），基础状态尚可")
if rhr <= 50: positives.append(f"静息心率 {rhr}bpm，心血管系统效率良好")
...
if recovery >= 85:
    ...
    if rhr_dev >= 3 and rhr_dev < 5: adv += "\nRHR 略有偏高，热身时多花几分钟，感觉好了再上强度。"
    if rhr_dev >= 5: adv += "\nRHR 偏高明显，今天建议做轻松有氧，不要做高强度间歇。"
    if not awake >= 3 and not ts < 6*3600 and not (rhr_dev >= 3): adv += "\n各项指标都在正常范围，享受训练吧！"
elif recovery >= 55:
...
print(f"║               {today.strftime('%Y-%m-%d')}  ·  星期四")
print("╚══════════════════════════════════════════════════════════════╝")
print()
...
if recovery >= 85: label = "🟢 状态极佳"
elif recovery >= 70: label = "🔵 状态良好"
elif recovery >= 55: label = "🟡 需要注意"
elif recovery >= 40: label = "🟠 恢复不足"
else: label = "🔴 需要休息"
print(f"  {label}")
print()
...
if ts >= 7*3600: print("✅ 充足（>7h）")
elif ts >= 6*3600: print("⚠️ 偏少（6-7h）")
else: print("🔴 严重不足（<6h）")
print(f"   深睡:    {deep//60}min ({dp:.0f}%)  ", end="")
if dp >= 10 and dp <= 25: print("✅ 理想范围 10-25%")
elif dp >= 5: print("⚠️ 偏低")
else: print("🔴 严重不足")
print(f"   REM:     {rem//60}min ({rp:.0f}%)  ", end="")
if rp >= 18 and rp <= 25: print("✅ 正常")
elif rp >= 10: print("⚠️ 偏低")
else: print("🔴 偏低")
print(f"   清醒:    {awake}次  ", end="")
if awake <= 1: print("✅ 正常（<2次）")
elif awake == 2: print("⚠️ 略多（2次）")
else: print("🔴 碎片化（≥3次）")
print()
...
try:
    ...
    if race_predictions:
        ...
        for k, v in race_predictions.items():
            distances = {"time5K": 5, "time10K": 10, "timeHalfMarathon": 21.0975, "timeMarathon": 42.195}
            names = {"time5K": "5K", "time10K": "10K", "timeHalfMarathon": "半马", "timeMarathon": "全马"}
            h = v // 60
            m = v % 60
