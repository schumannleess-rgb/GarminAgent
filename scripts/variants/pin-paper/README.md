# pin-paper — deep_diagnosis_pin_and_paper.html 风格变体
...
scripts/rebuild_kpi_today.py   (须 GARMIN_OUTPUT_DIR=output 覆盖，相对路径，Windows 安全)
  ...
  output/kpi_today.json              ← 数据源头（公开目录，被所有交付物读取）
         ↓
  scripts/variants/pin-paper/build.py   ← 读取完整 kpi_today.json → 注入 STATIC_DATA
         ↓ 写入
  output/html/deep_diagnosis_pin_and_paper.html
```
...
## 加载约定（对齐标准版 deep_diagnosis_latest.html）
