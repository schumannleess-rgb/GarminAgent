...
<body data-style="zine">
  ...
  <script>
    ...
    function drawLineData(ctx, data, labels, pad, w, h, color, fillBelow, titleText) {
      ...
      const xy = (v, i) => [pad.left + step * i, pad.top + ch * (1 - (v - min) / (max - min))];

      // fill (仅当无缺口时填充，避免跨缺口错误基线)
      if (fillBelow && vals.length === data.length) {
        ctx.beginPath();
        data.forEach((v, i) => { const [x, y] = xy(v, i); i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y); });
    ...
    function drawDualChart(hrvData, rhrData, labels) {
      ...
      const hrvMin = hrvVals.length ? Math.max(0, Math.min(...hrvVals) - 5) : 0;
      const hrvMax = hrvVals.length ? Math.max(...hrvVals) + 5 : 100;
      ctx.strokeStyle = '#E8E0D0'; ctx.lineWidth = 1;
      for (let i = 0; i <= 4; i++) {
...
def build_fallback_data(kpi: dict) -> dict:
    ...
    # 7 日趋势（改用 *_cal_28d，升序取末尾 7 条 = 最近 7 天；保留 null 缺口）
    hrv_trend = [e["value"] for e in (history.get("hrv_cal_28d") or [])[-7:]]
    rhr_trend = [e["value"] for e in (history.get("rhr_cal_28d") or [])[-7:]]

    sleep_hist = history.get("sleep_cal_28d") or []
    sleep_score_trend = []
    sleep_deep_trend = []
    for e in sleep_hist[-7:]:
        v = e.get("value") or {}
        gs = v.get("garmin_score")
        if gs is not None:
            sleep_score_trend.append(gs)
        elif v.get("total_sec"):
            sleep_score_trend.append(min(100, round(v["total_sec"] / 3600 / 9 * 100)))
        else:
            sleep_score_trend.append(0)
        ts = v.get("total_sec", 0)
        ds = v.get("deep_sec", 0)
        sleep_deep_trend.append(round(ds / ts * 100, 1) if ts > 0 else 0)

    rd_trend = []
    for e in (history.get("readiness_cal_28d") or [])[-7:]:
        rd_trend.append((e.get("value") or {}).get("score", 0))

    # 趋势标签
    hrv_dates = [e.get("date", "") for e in (history.get("hrv_cal_28d") or [])[-7:]]
    trend_labels = []
