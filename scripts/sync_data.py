...
def sync_health(client, lookback_days: int = 90):
    """同步健康数据：循环过去 N 天 → 写入 data/daily_health.json

    默认同步最近 90 天（原为 30 天），避免数据断档。
    已存在的日期会被增量更新。
    """
    ...
    for i in range(lookback_days):
        ...
        try:
            ...
            print(yellow(f"  ⚠️ 睡眠数据 [{date_str}]: {e}"))
        ...
        try:
            ...
            print(yellow(f"  ⚠️ HRV数据 [{date_str}]: {e}"))
        ...
        try:
            ...
            print(yellow(f"  ⚠️ 静息心率 [{date_str}]: {e}"))
        ...
        try:
            ...
            print(yellow(f"  ⚠️ 训练准备度 [{date_str}]: {e}"))
        ...
        try:
            ...
            print(yellow(f"  ⚠️ 压力数据 [{date_str}]: {e}"))
        ...
        if day_entry and has_core_data:
            day_entry["synced_at"] = today.isoformat()
            day_entry["updated_at"] = today.isoformat()
            days[date_str] = day_entry
            updated_count += 1

    health_data["metadata"]["last_sync"] = today.isoformat()
    health_data["metadata"]["total_days"] = len(days)
    _save_health_json(health_data)
    print(f"  💾 已写入 {_health_json_path()} (总计 {len(days)} 天, 本次更新 {updated_count} 天)")
