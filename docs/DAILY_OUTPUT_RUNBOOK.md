...
output/kpi_today.json                (★权威数据, 所有交付物读取, 不提交)
   ...
   ├─ output/html/recovery_standard.html    (标准版 · 双模式 · 手工维护, 不随脚本生成)
   ├─ output/html/recovery_pin_paper.html  (风格1: pin & paper)
   ├─ output/html/recovery_zine.html            (风格2: zine / Retro)
   ├─ output/html/recovery_data.html       (数字版 · 无样式全字段数据页)
   └─ output/html/recovery_swiss.html      (风格3: 每日恢复力日报 · Swiss deck, 由 scripts/ppt_common.py 共享内容层驱动)
```

要点：`kpi_today.json` 是唯一的"真相源"。HTML 都是它的视图，本身不存数据。
**每日一次数据更新 = 脚本重生成 4 个 HTML（3 风格 + 1 数字版），另加 1 个手工维护的标准版，共 5 个交付物。** 标准版 `recovery_standard.html` 是手工维护的基准，不随脚本生成，但 file:// 下的 STATIC_DATA 快照需手动重嵌。
...
`gen_trend_data_view.py`、`scripts/variants/recovery_deck/build.py`、以及 `scripts/variants/*/build.py` 都直接读公开 `output/kpi_today.json`，无需额外覆盖。
...
- **标准版是唯一逻辑源**：`recovery_standard.html` 带 `STANDARD` 标记，是数据/逻辑基准。风格变体只改 CSS、复用同一份 `kpi_today.json`，失败走固定失败页、绝不回退内嵌旧数据。改逻辑只改标准版。
...
- **trend_tracker.html 已归档**：内容不符合要求，已移至 `output/html/archive/trend_tracker.html`。其配对 `recovery_data.html`（无样式数据页）保留，作为每日 4 HTML 之一。若要彻底退役，把 `recovery_data.html` 也归档。
...
| `output/html/recovery_standard.html` | 标准版深度诊断（file:// 静态 + http 实时/失败页） | 手工维护（标准版） |
| `output/html/recovery_pin_paper.html` | 风格1：pin & paper（已对齐 STATIC_DATA 双模式） | `scripts/variants/pin_paper/build.py` |
| `output/html/recovery_zine.html` | 风格2：zine / Retro（已对齐 STATIC_DATA 双模式，2026-07-16 修复） | `scripts/variants/zine/build.py` |
| `output/html/recovery_data.html` | 无样式全字段数据页 | `gen_trend_data_view.py` |
| `output/html/recovery_swiss.html` | 每日恢复力日报 PPT（Swiss 皮肤，RECOVERY-DECK-STANDARD v2） | `scripts/variants/recovery_deck/build.py` + `scripts/ppt_common.py`（共享内容层） |
| `output/html/archive/` | 已归档旧交付物（trend_tracker / 旧 ppt / DEEP DIAGNOSIS v2.0） | — |

**每日数据更新产出的 5 个交付物** = 1 个手工标准版 + 1 个数字版 + 3 个风格 HTML：
- 标准版（手工）：`recovery_standard.html`
- 数字版（脚本）：`recovery_data.html`
- 风格1（脚本）：`recovery_pin_paper.html`
- 风格2（脚本）：`recovery_zine.html`
- 风格3（脚本）：`recovery_swiss.html`
...
- 跑变体测试确认数据已注入：
  ...
  ```
- （可选）一键校验全部 5 个交付物内嵌数据是否与 `output/kpi_today.json` 一致（Hermes `file://` 静态快照路径）：
  ```bash
  node scripts/verify_deliverables.js
  ```
- `output/html/recovery_swiss.html` 的 KPI（HRV / RHR / 睡眠 / 准备度 / 综合分）与 `kpi_today.json` 一致。
...
**只刷新数据页**：跑第 2 步。
**只刷新 PPT（deck）**：跑 `python scripts/variants/recovery_deck/build.py`。
**Hermes 离线看标准版**：重嵌 `STATIC_DATA` 到 `recovery_standard.html`（见 KPI_DATA_CONTRACT.md §2.1），无需起服务；风格变体离线看则重跑对应 `build.py`。
