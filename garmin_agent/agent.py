...
def _resolve_env(name, fallback=None, alt_name=None):
    """Resolve environment variable, supporting both ANTHROPIC_* and ZHIPU_* naming."""
...
class GarminAgent:
    ...
    def __init__(
        ...
        # Support both ANTHROPIC_* and ZHIPU_* naming conventions
        # ANTHROPIC_* takes priority (StepFun, etc.)
        self.api_key = api_key or _resolve_env("ANTHROPIC_AUTH_TOKEN", alt_name="ZHIPU_API_KEY")
        self.base_url = base_url or _resolve_env(
            "ANTHROPIC_BASE_URL",
            fallback="https://open.bigmodel.cn/api/anthropic",
            alt_name="ZHIPU_BASE_URL",
        )
        self.model = model or _resolve_env(
            "ANTHROPIC_DEFAULT_FABLE_MODEL",
            fallback="glm-4.7",
            alt_name="ZHIPU_MODEL",
        )
